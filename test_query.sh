...
SIGNATURE=$(echo -ne $DOCUMENT | openssl dgst -sha256 -sign $DIR/user.DTAG.key | openssl base64 | tr -d '\n')
...
# do the signing
CERT=$(cat $DIR/user.DTAG.crt_newline)
SIGNATURE=$(echo -ne $DOCUMENT | openssl dgst -sha256 -sign $DIR/user.TMUS.key | openssl base64 | tr -d '\n')
